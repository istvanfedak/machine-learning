...
// simple costructor for attribute frequency struct
AttrFreq::AttrFreq() {
  demYes = 0;
  repYes = 0;
  demNo  = 0;
...
Frequency::Frequency(const Data & randData, const unsigned int trainIndex) {
  this->reset(); 				// reset class
  this->interpolation(randData, trainIndex);	// calculate all the frequencies
}
...
// function that calculates frequencies
// randData is a random string matrix with all of the data
// trainIndex is the index of randData that will be used to train the naive bayesian clasifier
void Frequency::interpolation(const Data & randData, const unsigned int trainIndex) {
  this->reset();

  // if the training index is larger than the data print error
  ...
  for ( unsigned int i = 0; i < trainIndex; i++ )					// iterate through all of the training data
  { bool democrat;									// boolean used to specify if example is democrat
    for (unsigned int j = 0; j < randData[i].size(); j++ ) {
      if ( j == 0 ) {									// store class frequency
      ...
      else {										// store attribute frequency
        if ( randData[i][j] == "y" ) {
          if ( democrat )
            this->at(j).demYes += 1;
          else
